...
class PageTemplater
{
    ...
    public function create_checkout_page(){
        ...
        // $sql = ;
        $result = $wpdb->get_results($wpdb->prepare("SELECT * FROM %s AS p INNER JOIN %s AS meta ON p.ID = meta.post_id WHERE post_type = '%s' AND post_status='%s' AND (meta_value = '%s' OR meta_key = '%s')", $table_insert, $join_table, 'page', 'publish', 'mstore-checkout-template.php', '_mstore_checkout_template'),OBJECT);
        if(empty($result)){
